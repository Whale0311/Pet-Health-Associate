...
- **AI-powered Health Alerts**: Sử dụng mô hình Decision Tree để phân loại mức độ rủi ro sức khỏe
- **Cảnh báo Collar Detachment**: Phát hiện khi vòng cổ được tháo ra (nhịp tim < 40 bpm hoặc nhiệt độ < 30°C)
- **Anti-spam Alerts**: Cơ chế chống spam 10 giây để tránh thông báo trùng lặp
- **Alert Levels**: 
  - ⚠️ Level 1 (Warning): Cảnh báo sức khỏe bất thường
  - 🚨 Level 2 (Danger): Cảnh báo khẩn cấp tình trạng sức khỏe nghiêm trọng
- **Quản lý Thông báo**: Xem, đánh dấu đã đọc, xóa thông báo
- **Real-time Push**: Gửi thông báo tức thì qua Socket.IO
...
│   ├── healthRoutes.js         # API theo dõi sức khỏe & AI alerts
│   └── notificationRoutes.js   # API quản lý thông báo
├── server.js                    # Entry point chính, setup Express & Socket.IO
├── pet_ai_model.json           # 🤖 Mô hình Decision Tree AI (được load vào RAM)
...
| `routes/healthRoutes.js` | Endpoints: POST health data, AI predictions, auto-alerts |
| `routes/notificationRoutes.js` | Endpoints: GET notifications, mark as read, DELETE notification |
| `pet_ai_model.json` | Mô hình Decision Tree AI để phân loại mức độ rủi ro sức khỏe |
...
```json
{
  ...
  "alert": {
    "title": "⚠️ Health Alert (Warning)",
    "message": "Abnormal health signs detected. Heart Rate: 95 bpm, Temp: 38.5°C. Please check your pet."
  }
}
```
...
Hệ thống này sử dụng một **mô hình Decision Tree** được lưu trong `pet_ai_model.json` để phân loại mức độ rủi ro sức khỏe.

**Quy trình xử lý:**
1. **Nhận dữ liệu** từ thiết bị: nhịp tim, nhiệt độ, hành vi
2. **Kiểm tra Sensor**: Nếu nhịp tim ≤ 40 hoặc nhiệt độ < 30 → Cảnh báo **"Collar Detached"**
3. **Điều chỉnh nhiệt độ**: Thêm 1.5°C để ước lượng nhiệt độ lõi
4. **Chạy AI**: Duyệt cây quyết định với dữ liệu đã điều chỉnh
5. **Phát sóng Real-time**: 
   - Emit `new_health_data_{pet_id}` cho tất cả dữ liệu
   - Emit `new_alert_user_{user_id}` cho các cảnh báo

**Alert Levels:**
- **Alert Level 0**: Bình thường - Không có cảnh báo
- **Alert Level 1** ⚠️: **Warning** - Phát hiện dấu hiệu bất thường, khuyến cáo kiểm tra thú cưng
- **Alert Level 2** 🚨: **Danger** - Phát hiện tình trạng sức khỏe nghiêm trọng, cần kiểm tra ngay lập tức

**Anti-Spam Mechanism:**
- Chỉ tạo thông báo nếu không có thông báo giống trong 10 giây gần nhất
- Tránh làm phiền người dùng với thông báo trùng lặp
...
Xóa một thông báo cụ thể (chỉ người dùng sở hữu mới có thể xóa)
...
**Kiểm tra:**
- ✅ Chỉ người dùng sở hữu thông báo mới có thể xóa
- ✅ Trả về 404 nếu thông báo không tồn tại hoặc không có quyền
